...
import { getCartByOrderId, removeCartItem, updateCartItemQuantity, updateOrderDelivery } from '@/services/cartService'
...
const reducer = (state, action) => {
  switch (action.type) {
    ...
    case 'SUCCESS': {
      const adjustedDiscount = Math.min(state.discount, action.payload.subtotal)
      return {
        ...state,
        loading: false,
        ...
        total: calculateTotal(action.payload.subtotal, adjustedDiscount, action.payload.shippingFee),
        error: null,
      }
    }
    case 'FAILURE':
      return { ...state, loading: false, initialLoading: false, error: action.payload }
    case 'SET_ERROR':
...
export const useCart = (orderId) => {
  const { syncItems, notify } = useCartContext()
    ...
    getCartByOrderId(orderId)
      .then((order) => {
        const items = (order?.items || []).map((item) => {
          const unitPrice = typeof item.price === 'number' ? item.price : item.menuItem?.price || 0

          return {
            id: item.id,
            menuItemId: item.menuItemId,
            name: item.menuItem?.name || `Món #${item.menuItemId}`,
            description: item.menuItem?.description || 'Món ăn trong đơn hàng hiện tại',
        ...
        const subtotal = items.reduce((sum, current) => sum + current.subtotal, 0)
      ...
      .catch((error) => {
        dispatch({ type: 'FAILURE', payload: error.message || 'Không thể tải giỏ hàng' })
      })
  }, [orderId, syncItems])
